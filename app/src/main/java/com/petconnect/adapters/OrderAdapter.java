...
public class OrderAdapter extends RecyclerView.Adapter<OrderAdapter.ViewHolder> {
    ...
    private String currentUserId; // Ajout pour stocker l'ID utilisateur
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        // Ajout du clic pour voir la facture
        holder.itemView.setOnClickListener(v -> {
            Intent intent = new Intent(context, InvoiceActivity.class);
            intent.putExtra("orderId", order.getId());

            // Utiliser l'ID utilisateur stocké ou une valeur par défaut
            String userIdToSend = (currentUserId != null && !currentUserId.isEmpty())
                    ? currentUserId
                    : "user_id_here"; // Valeur par défaut ou récupérer dynamiquement
    ...
    // Méthode pour mettre à jour la liste des commandes
    ...
    private String generateOrderHtml(Order order) {
        ...
        if (order.getItems() != null) {
            for (com.petconnect.services.CartService.CartItem item : order.getItems()) {
                html.append("<tr>");
