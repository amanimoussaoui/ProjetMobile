...
public class CartService {
    ...
    /**
     * Sauvegarder le panier complet dans Firebase (pour synchronisation complète)
     */
    private void saveCartToFirebase(CartCallback callback) {
        Log.d("CartService", "Sauvegarde panier complet dans Firebase");
        ...
        for (CartItem item : cartItems) {
            if (item == null || item.product == null || item.product.getId() == null) {
                savedCount[0]++;
                Log.w("CartService", "Item invalide ignoré lors de la sauvegarde");
                if (savedCount[0] == totalItems && callback != null) {
                    if (allSaved[0]) {
                        callback.onSuccess();
                    } else {
                        callback.onError("Erreur sauvegarde certains items");
                    }
                }
                continue;
            }
            ...
            db.collection("PetConnect")
                    .document("cart")
                    .collection(userId)
                    .document(item.product.getId())
                    .set(itemData)
                    .addOnSuccessListener(aVoid -> {
                        savedCount[0]++;
                        Log.d("CartService", "Item sauvegardé: " + item.product.getName());

                        if (savedCount[0] == totalItems) {
                            if (allSaved[0] && callback != null) {
                                callback.onSuccess();
                            }
                        }
                    })
                    .addOnFailureListener(e -> {
                        Log.e("CartService", "Erreur sauvegarde item: " + e.getMessage());
                        allSaved[0] = false;
