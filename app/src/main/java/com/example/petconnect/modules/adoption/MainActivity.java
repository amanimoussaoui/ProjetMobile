...
import com.example.petconnect.modules.adoption.repository.PetRepository;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // ✅ AJOUTER LES DONNÉES DE TEST SI C'EST LA PREMIÈRE FOIS
        addTestDataIfNeeded();
    ...
    /**
     * Ajoute les données de test la première fois
     */
    private void addTestDataIfNeeded() {
        FirebaseFirestore db = FirebaseFirestore.getInstance();
        
        // Vérifier si les pets existent déjà
        db.collection("pets")
            .limit(1)
            .get()
            .addOnSuccessListener(querySnapshot -> {
                if (querySnapshot.isEmpty()) {
                    Log.d(TAG, "📦 Collection vide - ajout des données de test...");
                    FirestoreDataHelper.addAllPetsToFirestore(db);
                    Toast.makeText(MainActivity.this, "📦 Données de test en cours d'ajout...", Toast.LENGTH_SHORT).show();
                } else {
                    Log.d(TAG, "✅ Collection contient déjà des données");
                }
            })
